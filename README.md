...
1. Find the name of the relevant connection, e.g. for "eth0"
    ...
    nmcli -p c
    ```
    Let's assume the name is `Wired connection 1`. ***This value is used as an example in the following commands.***

2. Show the **ipv6.addr-gen-mode** configuration for the selected connection.
    ```bash
    # Replace 'Wired connection 1' with the relevant value from step 1
    nmcli -p c show "Wired connection 1" | grep "ipv6.addr-gen-mode"
    ```

4. If the value is "stable-privacy", then a new IID is generated by the client each time the ISP reassigns the IPv6 prefix.  
...
5. **Make sure that this value is not used by any other client in your network.**  
...
6. **Set the stable IID on the client**  
> [!WARNING]  
